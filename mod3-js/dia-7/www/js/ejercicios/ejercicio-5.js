...
function mitadYMitad(menu) {
    for (let mitad of menu) {
        let mitadIndex = menu.indexOf(mitad);
        for (let i = mitadIndex + 1; i < menu.length; i++) {
            if (menu[mitadIndex] !== menu[i]) {
                salida.push(`${menu[mitadIndex]} y ${menu[i]}`);
            } else {
                salida.push(`${menu[mitadIndex]}`);
            }
        }
    }
    return salida;
